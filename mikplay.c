...
#define MEM_LOAD_THRESHOLD 6291456  // load to memory if smaller than 6 megs
#define VER_MAJ 0
#define VER_MIN 3
...
int main(int argc, char *argv[])
{
    ...
    printf("Type: %s\nChannels: %d, Patterns: %d, Instruments: %d, Samples: %d\n\nName: %s\n", module->modtype, module->numchn, module->numpos, module->numins, module->numsmp, module->songname);
    ...
    while (1)
    {
        ...
        if (kbhit())
        {
            ...
            if (ch == 27 || ch == 81 || ch == 113) break; // ESC or Q or q
            else if (ch == ' ')
            { // space
                Player_TogglePause();
                update_status(module, current_volume);
